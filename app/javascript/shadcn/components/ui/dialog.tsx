...
const DialogOverlay = React.forwardRef<React.ElementRef<typeof DialogPrimitive.Overlay>, DialogOverlayProps>(
	({ className, closeFn, ...props }, ref) => (
		<DialogPrimitive.Overlay
			ref={ref}
			onClick={closeFn}
			className={cn(
				"data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 custom-blur fixed inset-0 z-50",
				className,
			)}
			{...props}
		/>
...
const DialogContent = React.forwardRef<React.ElementRef<typeof DialogPrimitive.Content>, DialogContentProps>(
	({ className, children, closeFn, showXButton = true, ...props }, ref) => (
		<DialogPortal>
			<DialogOverlay closeFn={closeFn} />
