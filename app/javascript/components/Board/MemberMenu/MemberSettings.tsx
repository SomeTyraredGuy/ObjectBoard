...
function MemberSettings({ currentMember, member, closeFn, refetchFn }: Props) {
	...
	const buttons = [
		...
		{ name: "Save", onClick: () => save(newRole), disabled: changesIsDisabled() },
	];
	const roleNames = [
		{ name: "Owner", disabled: true },
		{ name: "Admin", disabled: !currentMember.role.can_assign_admin },
		{ name: "Editor", disabled: !currentMember.role.can_change_roles },
		{ name: "Viewer", disabled: !currentMember.role.can_change_roles },
	];
	...
	return (
		<CenterMenu buttons={buttons} closeFn={closeFn} className={`${classes.maxwidthSmall}`}>
			<h2 className="text-center m-3">{member.name}</h2>
			<ul className="list-group">
				<li className="list-group-item">
					<Selector
						options={roleNames}
						currentOption={newRole.name}
						setOption={setNewRoleByName}
				...
				<li className="list-group-item">
					<SwitchCheck
						text={"Can edit"}
						value={newRole.can_edit}
						setValue={(value) => setNewRole({ ...newRole, can_edit: value })}
						disabled={true}
					/>
				</li>
				<li className="list-group-item">
					<SwitchCheck
						text={"Can ignore rules"}
						value={newRole.can_ignore_rules}
						setValue={(value) => setNewRole({ ...newRole, can_ignore_rules: value })}
						disabled={true}
					/>
				</li>
				<li className="list-group-item">
					<SwitchCheck
						text={"Can change roles"}
						value={newRole.can_change_roles}
						setValue={(value) => setNewRole({ ...newRole, can_change_roles: value, can_assign_admin: false })}
						disabled={adminChangesIsDisabled()}
					/>
				</li>
				<li className="list-group-item">
					<SwitchCheck
						text={"Can assign admin"}
						value={newRole.can_assign_admin}
						setValue={(value) => setNewRole({ ...newRole, can_assign_admin: value })}
						disabled={adminChangesIsDisabled()}
					/>
				</li>
			</ul>
